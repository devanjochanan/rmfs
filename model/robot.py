...
class Robot(Object):
    ...
    def drawNextPosition(self):
        ...
        if self.velocity != 0:
            ...
            dim = self.universe.landscape.dimension
            self.pos_x = max(0, min(dim, self.pos_x))
            self.pos_y = max(0, min(dim, self.pos_y))
        self.coordinate = NetLogoCoordinate(round(self.pos_x), round(self.pos_y))
    ...
    def _find_nearest_graph_node(self, graph, x, y):
        """Find the nearest valid graph node to (x, y) by searching nearby cells."""
        for radius in range(1, 10):
            for dx in range(-radius, radius + 1):
                for dy in range(-radius, radius + 1):
                    if abs(dx) != radius and abs(dy) != radius:
                        continue  # only check the perimeter
                    key = self.coordinate_to_string_key(x + dx, y + dy)
                    if key in graph.graph.nodes:
                        return key
        return None
    ...
    def set_move(self, dest: NetLogoCoordinate, graph, need_neutralize_robot: bool = False, avoid_side: bool = False):
        ...
        # If start node isn't in the graph, snap to nearest valid node
        if start not in graph.graph.nodes:
            nearest = self._find_nearest_graph_node(graph, round(self.pos_x), round(self.pos_y))
            if nearest is not None:
                print(f"[WARN] Robot {self._id} at {start} not in graph, snapping to {nearest}")
                start = nearest
                # Update robot position to match
                parts = nearest.split(',')
        ...
        # If no route found (e.g. robot on unreachable node), retry without avoidance
        if node_routes is None and nodes_to_avoid:
            node_routes = graph.dijkstra(start, end)

        if node_routes is None:
            print(f"[WARN] No path from {start} to {end}, robot {self._id} will idle")
            return
